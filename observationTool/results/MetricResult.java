...
public class MetricResult {
	...
	public MetricResult(String sysName, String metricName, int totalNumberOfSteps, SystemInfo si){
		...
		resultsDir += sysName.replaceAll("\\s+","")+"/"+metricName.replaceAll("\\s+","")+"/best/"+si.getConfigurationString()+"_";
	}
	...
	public double accuracyCalculation(String resType1, String resType2, double thresholdT1, double thresholdT2, int window, boolean saveToFile){
...
//			}
		...
		//We need to send to file the positive detections against the actual detections
			...
			Utilities.writeToFile(toFileString, resultsDir+resType1.replaceAll("\\s+","")+"_"+resType2.replaceAll("\\s+","")+".tsv");
		}
...
//		System.out.println("max length: "+maxLength);
		...
		for (int i = 0; i < maxLength; i++){
			...
			if (stringResultsCalculated[i].compareToIgnoreCase(sMatch1) == 0 && stringResultsReal[i].compareToIgnoreCase(sMatch2) == 0){
				hits++;				
			} 
		}
