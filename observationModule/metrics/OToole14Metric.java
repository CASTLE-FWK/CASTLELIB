	...
	public void run(ArrayList<VEntity> agents, int currentStep){
		...
		for (VEntity xGrid[] : rawGrid){
			for (VEntity agt : xGrid){
				...
				ArrayList<VEntity> neighbours = (ArrayList<VEntity>) theGrid.getNeighbours((int)agt.getPosition().getX(), (int)agt.getPosition().getY(), 1);
		...
		if (currentWindowSize == maxWindowSize){
			...
			//If current window size is 20 (or N)
				...
				while (it.hasNext()){
					...
					//Calculate the covariance:  sum(t->N) ((Xt - Xhat) * (Yt - Yhat) / N)
					int[] xValues = aw.getX();
					int[] yValues = aw.getY();
					double cov = 0.0;
					...
					//Calculate the product moment coeffecient: cov(x,y) / sqrt(stddev(x))*sqrt(stddev(y))
