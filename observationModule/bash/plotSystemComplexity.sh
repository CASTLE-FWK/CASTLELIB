...
gnuplot <<- EOF
		...
		set output "${FILE}.pdf" 
		set title "System Complexity: ${name} ${stats}"
		set key off

		# set style data dots pointsize 5
		# plot "${FILE}" using 1:2 with lines  #pointtype 7 pointsize 0.2 lt rgb "red" title ""
		plot "${FILE}" using 1:2 with lines, \
		"" using 1:(column(3) * (0.5 * scMax)) with points pointtype 7 pointsize 0.1
EOF
