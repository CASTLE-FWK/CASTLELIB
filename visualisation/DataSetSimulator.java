 ...
 * Well, we take a dataset ID, find out how long the simulation ran
 * 	Draw agents on a window (define their colour based on a state [Hardcode for now])
 * 
 * 
 * 
...
//			drawSimpleMetrics(time, agents, collector);
	...
	public void drawAgentOnGrid(VEntity agt){
		...
		//Or Phorcys MeshPane
//		mesh.addEntityToMap(agt.getName(), pos.getX()/dimensions.getX(), pos.getY()/dimensions.getY(), 0, colourAsThreeString);
...
//		
		...
		entPlot = new Plot2DHelper("Entropy","Time","Entropy","Time","Shannon Entropy", "Shannon Entropy (SC)","Conditional Entropy");
		...
		TTPlot = new Plot2DHelper("TagAndTrack","Time","Result","Time","averageArea","clustersIntersecting","RunningUniqueClusters","averageAgentDensity");
	}
	...
	/***** METRIC PRINTING ******/
	void drawSimpleMetrics(int time, ArrayList<VEntity> currAgents, DataCollector collector){
		//System Complexity
